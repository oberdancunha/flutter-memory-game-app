...
#### Principio da inversão de dependência (Dependence Inversion Principle - DIP)

No entanto, o enfoque será em como o princípio da inversão de dependência possibilitou a escalabilidade desse projeto.

Como o projeto é somente um jogo de memória e não uma plataforma de jogos diversos (quem sabe futuramente possa vir a ser), a sua escalabilidade está voltada na disponibilidade dos tipos de cartas para o jogo. Atualmente o jogo dispõe de 4 tipos de cartas.

É exatamente nesse ponto que é necessário ressaltar o DIP.

Analisando a figura da arquitetura do projeto, é possível visualizar a camada Data. É nessa camada onde os tipos de cartas do jogo estão armazenados. Cada tipo é retornado para o repositório usando uma classe intermediária chamada CardDto.
...
O acesso é realizado usando a inversão de dependência. O código abaixo mostra como a classe CardRepository é criada.
...
<b>CardDataSource</b> é uma classe abstrata, ou seja, CardRepository recebe em seu construtor uma abstração e não uma implementação, exatamente como o DIP descreve.
...
É possível perceber, na declaração da classe, que CountryFlagsDataSource implementa a classe abstrata CardDataSource. Em outras palavras, CountryFlagsDataSource é uma implementação de CardDataSource. Em resumo, qualquer instância da classe concreta CountryFlagsDataSource também será compatível com o tipo CardDataSource.
...
Caso seja necessário um novo tipo de carta para o jogo, como, por exemplo, o tipo frutas, basta criar uma classe chamada <b>FruitsDataSource</b>, com a lista das frutas e injetar essa classe em CardRepository.
...
Para cada tipo de carta escolhida, o aplicativo recarrega a mesma rota, porém passando como parâmetro a instancia da respectiva classe. O flutter modular aceita realizar a injeção com a instância da classe recebida do parâmetro, facilitando demais o trabalho.

Além disso, ao recarregar a mesma rota, o flutter modular automaticamente realiza a limpeza (<i>dispose</i>) dos dados antigos da memória.
